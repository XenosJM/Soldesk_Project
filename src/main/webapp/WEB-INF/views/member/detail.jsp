...
<body>
	<h2>${memberName}님의 정보입니다.</h2>
	<p>현재 이메일 : ${memberEmail }</p><br>
					
	<c:choose>
		<c:when test="${memberProperty eq null }"><label for="buyProperty">현재 보유중인 상품이 없습니다. 구매하러 가시겠습니까?<!-- //TODO 변경할 예정 -->
	...
	<p>회원가입 날짜 : ${memberRegistDate }</p><br>
	...
	<form id="modifyForm">	
		...
		<c:when test="${memberProperty eq null }"><label for="buy">현재 보유중인 상품이 없습니다. 구매하러 가시겠습니까?
			<input id="buy" type="submit" onclick="location.href='http://localhost:8080/ex01/shop/imoji'" value="상점"></label><br></c:when>
		<c:when test="${memberProperty ne null }"><p>현재 보유중인 상품 목록 : ${memberProperty }</p><br></c:when>
		</c:choose>
